    ...
    function __construct()
    {
        ...
        if (defined('IN_ADMINCP')) {
            ...
            $plugins->add_hook(
                ...
                array($this, 'hook_admin_forum_management_edit_commit')
            );
            $plugins->add_hook(
...
._positive {
            ...
            foreach ($fields as $name => $definition) {
                if (!$db->field_exists($name, $table)) {
                    ...
                    if ($table == 'usergroups') {
                        ...
                        if (in_array(
                            $name,
                            array('ougc_feedback_ismod', 'ougc_feedback_mod_canedit', 'ougc_feedback_mod_canremove')
                        )) {
                            $db->update_query('usergroups', array($name => 1), "gid='4'"); // Administrators
    ...
    function load_pluginlibrary()
    {
        ...
        if (!file_exists(PLUGINLIBRARY)) {
            flash_message(
                $lang->sprintf(
                    ...
                    self::$plugin_info['pl']['ulr'],
                    self::$plugin_info['pl']['version']
                ),
                'error'
            );
        ...
        if ($PL->version < self::$plugin_info['pl']['version']) {
            ...
            flash_message(
                $lang->sprintf(
                    $lang->ougc_feedback_pluginlibrary_old,
                    $PL->version,
                    self::$plugin_info['pl']['version'],
                    self::$plugin_info['pl']['ulr']
                ),
            ...
            foreach ($db_fields['usergroups'] as $name => $definition) {
                if ($name == 'ougc_feedback_maxperday') {
                    ...
                    $perms[] = $form->generate_check_box(
                        ...
                        1,
                        $lang->{$lang_var},
                        array('checked' => $mybb->get_input($name, 1))
                    );
                }
            }
